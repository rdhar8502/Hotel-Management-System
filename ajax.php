...
if (isset($_GET['delete_room'])) {
	...
	$sql = "DELETE FROM `room` WHERE `room`.`room_id` = '$room_id' AND status IS NULL";
    $result = mysqli_query($connection, $sql);
    if ($result) {
        header("Location:room_mang.php?success");
    } else {
        header("Location:room_mang.php?error");
    }
}
...
if (isset($_POST['createComplaint'])) {
    ...
    $result = mysqli_query($connection, $query);
    if ($result) {
        header("Location:user_complain.php?success");
    } else {
        header("Location:user_complain.php?error");
    }
...
if (isset($_POST['createFeedback'])) {
    ...
    $feedback = $_POST['feedback_desc'];

    $query = "INSERT INTO `feedback` (feedback_name, feedback_contact, feedback) VALUES ('$feedback_name','$feedback_contact','$feedback')";
    $result = mysqli_query($connection, $query);
    if ($result) {
...
if (isset($_POST['change_shift'])) {
    ...
    if ($result && $insert_result && $update_result) {
        header("Location:staff_mang.php?success");
    } else {
        header("Location:staff_mang.php?error");
    }
}
